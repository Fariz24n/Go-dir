...
func SetupRoutes(db *gorm.DB) http.Handler {
	...
	r.HandleFunc("/gudang", handlers.CustomerHandler(db)).Methods("GET")
	r.HandleFunc("/move-stock", handlers.MoveStock(db)).Methods("POST")
	r.HandleFunc("/move-carts", handlers.MoveCarts(db)).Methods("POST")

	r.HandleFunc("/login", operasi.Login(db)).Methods("POST")
	r.HandleFunc("/register", operasi.Register(db)).Methods("POST")
	r.HandleFunc("/logout", operasi.Logout(db)).Methods("POST")

	r.HandleFunc("/move-cart", UserOnly(handlers.MoveCart(db))).Methods("POST")
	r.HandleFunc("/purchase", UserOnly(operasi.PurchaseHandler(db))).Methods("POST")
	r.HandleFunc("/gudang", AdminOnly(operasi.AddGudang(db))).Methods("POST")
	r.HandleFunc("/move-stock", AdminOnly(operasi.MoveStock(db))).Methods("POST")
